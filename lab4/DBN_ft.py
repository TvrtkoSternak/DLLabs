...
if __name__=='__main__':
    with PdfPages('zad3_results.pdf') as pdf:
        ...
        plt.title('vizualizacija težina prvog sloja prije fine tuninga')
        pdf.savefig()
        ...
        plt.title('vizualizacija težina skrivenog sloja prije fine tuninga')
        pdf.savefig()
        ...
        with g3.as_default():
            ...
            h2_down = h2_up
            for step in range(gibbs_sampling_steps):
                h1_down_prob = tf.sigmoid(tf.matmul(h2_down, tf.transpose(w2a)) + hb1_down)
            ...
            update_w1_down = tf.assign_add(w1_down,
                                           beta * tf.matmul(tf.transpose(h1_up), X3 - v1_up_down_prob) / tf.to_float(
                                               tf.shape(X3)[0]))
        ...
        plt.title('vizualizacija R težina')
        pdf.savefig()
        draw_weights(w1_downs.T, v_shape, Nh, h1_shape)
        plt.title('vizualizacija W težina')
        pdf.savefig()
        draw_weights(w2ss, h1_shape, Nh2, h2_shape)
        plt.title('vizualizacija težina skrivenog sloja')
        pdf.savefig()
        ...
        r_input[r_input > 0.8] = 1  # postotak aktivnih - slobodno varirajte
        r_input[r_input < 1] = 0
        ...
        out_3 = sess3.run((v1_down), feed_dict={h2_up: r_input})

        # Emulacija dodatnih Gibbsovih uzorkovanja pomoću feed_dict
        for i in range(1000):
            out_1_prob, out_1, hout1 = sess3.run((v1_down_prob, v1_down, h1_down_up), feed_dict={X3: out_3})

        draw_generated(r_input, hout1, out_1_prob, v_shape, h1_shape, 20)
        plt.title('Generirani uzorci')
