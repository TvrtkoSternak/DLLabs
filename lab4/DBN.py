...
if __name__=='__main__':
    with PdfPages('zad2_results.pdf') as pdf:
        ...
        plt.title('vizualizacija težina prvog sloja')
        pdf.savefig()
        ...
        plt.title('vizalizacija težina drugog sloja')
        pdf.savefig()
        ...
        out_2 = sess2.run((v_out), feed_dict={h2up: r_input})

        # Emulacija dodatnih Gibbsovih uzorkovanja pomoću feed_dict
        for i in range(1000):
            out_1_prob, out_1, hout1 = sess2.run((v_out_prob, v_out, h1down), feed_dict={X2: out_2})
