...
with tf.name_scope('z'):
    ...
    z_log_sigma_sq = vae_layer(layer_e2, n_hidden_recog_2, n_z, 'z_std', act=tf.identity)
...
with tf.name_scope('cost'):
    ...
    cost2 = 1 + z_log_sigma_sq - tf.square(z_mean) - tf.exp(z_log_sigma_sq)
    tf.summary.histogram('D_KL', cost2)
    cost = tf.reduce_mean(tf.reduce_sum(cost1, 1) + -0.5*tf.reduce_sum(cost2, 1))  # average over batch
...
with PdfPages('zad4_results.pdf') as pdf:
    ...
    plt.title('vizualizacija rekonstrukcije i stanja')
    pdf.savefig()
    ...
    plt.title('Vizualizacija raspored testnih uzoraka u 2D prostoru skrivenih varijabli - 1. način')
    pdf.savefig()
    ...
    plt.title('Vizualizacija raspored testnih uzoraka u 2D prostoru skrivenih varijabli - 2. način')
    ...
    plt.title('Vizualizacija ugašenih elemenata skrivenog sloja - 1. način')
    ...
    def bargraph_vis(pos, input_data, dims, color, labels):
        ...
        ax.bar3d(xpos, ypos, zpos, len(dx), len(dy), len(dz), color=color)
