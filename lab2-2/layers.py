...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    # TODO
    self.cache = inputs
    out = inputs.dot(self.weights.T) + self.bias.T
    return out
  ...
  def backward_params(self, grads):
    ...
    """
    # TODO
    grad_weights = self.cache.T.dot(grads).T
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    loss = -np.sum(np.log(probs[np.arange(N), np.argmax(y, axis=1)])) / N
    return loss
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    # TODO
    return self.weight_decay * np.sum(np.power(self.weights, 2))/2
